...
class EnvironmentVariables:
    ...
    DH_ADMIN_PORT: Optional[int] = (
        int(os.getenv("DH_ADMIN_PORT")) if os.getenv("DH_ADMIN_PORT") else None
    )
    DH_ADMIN_HEALTH_ENDPOINT: Optional[str] = os.getenv("DH_ADMIN_HEALTH_ENDPOINT")

    DH_RAG_QUERY: Optional[str] = os.getenv("DH_RAG_QUERY")
    DH_RAG_QUERY_PORT: Optional[int] = (
        int(os.getenv("DH_RAG_QUERY_PORT")) if os.getenv("DH_RAG_QUERY_PORT") else None
    )
    DH_RAG_QUERY_HEALTH_ENDPOINT: Optional[str] = os.getenv(
        "DH_RAG_QUERY_HEALTH_ENDPOINT"
    )

    DH_AVATAR: Optional[str] = os.getenv("DH_AVATAR")
    DH_AVATAR_PORT: Optional[int] = (
        int(os.getenv("DH_AVATAR_PORT")) if os.getenv("DH_AVATAR_PORT") else None
    )
    ...
    WEAVIATE_PORT: Optional[int] = (
        int(os.getenv("WEAVIATE_PORT")) if os.getenv("WEAVIATE_PORT") else None
    )
    WEAVIATE_HEALTH_ENDPOINT: Optional[str] = os.getenv("WEAVIATE_HEALTH_ENDPOINT")

    # Logging and config
    LOG_LEVEL: Optional[str] = os.getenv("LOG_LEVEL")
    HTTP_REQUEST_TIMEOUT: Optional[int] = (
        int(os.getenv("HTTP_REQUEST_TIMEOUT"))
        if os.getenv("HTTP_REQUEST_TIMEOUT")
        else None
    )
    SERVICE_HEALTH_CHECK_INTERVAL: Optional[int] = (
        int(os.getenv("SERVICE_HEALTH_CHECK_INTERVAL"))
        if os.getenv("SERVICE_HEALTH_CHECK_INTERVAL")
        else None
    )
    RETRY_WAIT_SECONDS: Optional[int] = (
        int(os.getenv("RETRY_WAIT_SECONDS"))
        if os.getenv("RETRY_WAIT_SECONDS")
        else None
    )
